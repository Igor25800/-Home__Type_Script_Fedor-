let form:any = document.forms['form']
let tbody:HTMLElement = document.querySelector('.tbody')
...
class User implements IUser {
    ...
    emeilVal = form.emeil.value

    block(block:string , none:string) {
...
function saveEditUser():void{
   let user = new User()
    ...
    arr.forEach((el, i)=>{
     if (i === +currentId) {
        if (el.emeilVal !== email) el.emeilVal = email
        if (el.passwordVal !== password) el.passwordVal = password
        if (el.text !== formText) el.text = formText
        }
    })
    render()
