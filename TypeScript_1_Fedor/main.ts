...
let form: HTMLFormElement  = document.forms['form']
let text : HTMLElement = document.querySelector('.block__row')
let textRed:NodeList  = document.querySelectorAll('.red')
let textArea: HTMLElement = document.getElementById('textAria')

let arr:Array<string> = ['java', 'tottnham']
...
form.btnAdd.addEventListener('click', function(){
    let inpValue:string = form.intValue.value
...
form.btnCenzor.addEventListener('click', function(){
    let texts:string =  form.textArea.value
    const areaText:Array<string>= texts.split(/[.,\/ -]/)
    arr.filter(elMasiv => {
